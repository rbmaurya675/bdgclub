...
function showListOrder3(list_orders, x) {
    ...
    $(".game-list .con-box:eq(".concat(x, ") .hb1")).prepend(htmls);
    $(".game-list .con-box:eq(".concat(x, ") .hb1 .c-tc")).last().remove();
}
var socket = io();
var pageno = 0;
var limit = 10;
...
socket.on("data-server-rex", function (msg) {
    if (msg.data[0].type != 1) return;
    console.log("msg is trx...", msg)
    console.log("mes data as trx...", msg.data[0])
    $(".Loading").fadeIn(0);
    setTimeout(function () {
...
function showListOrder(list_orders, x) {
    ...
    $(".game-list .con-box:eq(".concat(x, ") .hb1")).html(htmls);
}
function showListOrder_t(list_orders, x) {
    ...
    var $targetDiv = $(".game-list .con-box:eq(".concat(x, ") .hb1"));
    $targetDiv.empty();
